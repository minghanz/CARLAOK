...
def _push_rule_based(obs,q_function,RL_action,testing=True):

    #print(obs[0,4],obs[0,7],obs[0,9],obs[0,11])
    if RL_action == 0:
        return True
...
def learn(env,
          ...
          print_freq=100,
          checkpoint_freq=500,
          checkpoint_path="~/models/carlaok_checkpoint/",
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        for t in range(total_timesteps):
            ...
            if done and print_freq is not None and len(episode_rewards) % print_freq == 0:
                ...
                logger.dump_tabular()
                num_str = str(num_save)
                rew_str = str(mean_100ep_reward)
                path = osp.expanduser("~/CARLAOK/Carla_model/"+num_str+"_"+rew_str)
                act.save(path)

            #print(checkpoint_freq, t,learning_starts,num_episodes)
