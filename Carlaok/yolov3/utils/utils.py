...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    if label:
        ...
        cv2.putText(img, 'lo %.1f'%(pos[1]), ((x[0] + x[2])/2, x[3]+5), 0, tl/3 - 0.2, [255, 0, 0], thickness=tf, lineType=cv2.LINE_AA)
        cv2.putText(img, 'la %.1f'%(pos[0]), ((x[0] + x[2])/2, x[3]+20), 0, tl/3 - 0.2, [255, 0, 0], thickness=tf, lineType=cv2.LINE_AA)

        return pos
    else:
        return None

def cal_real_pos(x, y, im_shape):
    # for 640*480, FOV 90
    FX = 320
    FY = 320
    CX = 320
    ...
    IM_X = im_shape[1]
    IM_Y = im_shape[0]
    alpha_wl = np.arctan2(CX, FX)
    alpha_wr = np.arctan2(IM_X - CX, FX)
    alpha_h = np.arctan2(IM_Y - CY, FY)
    van_x = CX
    van_y = CY
    theta_w = 0
    theta_h = 0
    h = 2.5
    axis_dist_bottom = h/np.sin(alpha_h - theta_h)
    real_width_bottom = axis_dist_bottom * np.cos(alpha_h) * (np.tan(alpha_wl) + np.tan(alpha_wr))
    real_dist_bottom = h/np.tan(alpha_h - theta_h)
    px_per_m_bottom = IM_X / real_width_bottom
    px_per_m_at_y = (y - van_y)/(IM_Y - van_y) * px_per_m_bottom
    if px_per_m_at_y == 0:
        px_per_m_at_y = 0.0001
    m_at_x = (x - CX)/px_per_m_at_y
    alpha_h_top = np.arctan2(y - CY, FY)
    if alpha_h_top == 0:
        alpha_h_top = 0.0001
    real_dist_top = h / np.tan(alpha_h_top - theta_h)
    m_at_y = real_dist_top
    return m_at_x, m_at_y
