...
class Driving_State(object):
    ...
    def __init__(self):
        ...
        self.ego_x = 0.0    ### GPS
        self.ego_y = 0.0    ### GPS
        self.ego_speed = 0.0  ### GPS
        self.ego_vehicle = None  ### cannot be used directly
        ...
        self.behind_vehicle_inside_direction = 0.0  ### cannot be used directly

        self.behind_vehicle_outside = None  ### cannot be used directly
        self.behind_vehicle_outside_distance = 0.0
        self.behind_vehicle_outside_direction = 0.0  ### cannot be used directly
        ...
        for lid in all_matched: # all_matched:
            ...
            if lid < len(lidar_detections):
                world_decision.debug.draw_point(carla.Location(x=lx,y=ly,z=1),
                    size=0.1, color=self._marker_color_lidar, life_time=0.2)
            else:
                world_decision.debug.draw_point(carla.Location(x=lx,y=ly,z=1),
...
#############################################################
        ...
        cx = self._cx
        cy = self._cy
        for target_vehicle, sid in matched_car: # vehicle_list:
            if target_vehicle.id == self._vehicle.id:
                continue
            if target_vehicle is None:
                continue
            # loc = target_vehicle.get_location()
            loc = carla.Location(x=sensor_detections[sid, 1], y=sensor_detections[sid, 2], z = 0)
            distance_to_roundabout = self._distance_to_center(target_vehicle)
            if distance_to_roundabout > 160:
    ...
    def run_step(self, debug=True):
        ...
        if debug:
            ...
            if self._inroundabout_inside:
                ...
                begin.z = 0.0
                world_draw.debug.draw_arrow(begin, end, arrow_size=1, life_time=1)
